sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	return BaseController.extend("com.sap.build.standard.prototipo1.controller.EstadoOrdenesDeCompra", {
		...
		_onObjectListItemPress: function (oEvent) {
			...
			return new Promise(function (fnResolve) {
				this.doNavigate("DetalleOdc", oBindingContext, fnResolve, "");
			}.bind(this)).catch(function (err) {
				if (err !== undefined) {
					MessageBox.error(err.message);
				}
		...
		_onSearchFieldLiveChange: function (oEvent) {
			var sControlId =
				"sap_m_Page_0-content-sap_m_ObjectList-1588887640354-nouaf9rif5yv9lqnor526f2b7_S7-uly6wf0jplbqultccjbee1b78_S8-d4bek9cybf5m7nd7i0r8ob0011_S11";
			...
			return new Promise(function (fnResolve) {
				...
				if (sQuery && sQuery.length > 0) {
					...
					if (!isNaN(iQuery)) {
						aFilters.push(new sap.ui.model.Filter("Unidades", sap.ui.model.FilterOperator.EQ, sQuery));
					}
			...
			return new Promise(function (fnResolve) {
				...
				.then(function (result) {
					...
					var confirmHandler = function (oConfirmEvent) {
						...
						vValueLT = oDialog.getModel().getProperty("/Unidades/vValueLT");
						vValueGT = oDialog.getModel().getProperty("/Unidades/vValueGT");
						if (vValueLT !== "" || vValueGT !== "") {
							oFilter = this.getCustomFilter("Unidades", vValueLT, vValueGT);
							oBindingData.filters.push(oFilter);
							sFilterString = sFilterString ? sFilterString + ', ' : 'Filtered by: ';
							sFilterString += this.getCustomFilterString(true, "Unidades", oFilter.sOperator, vValueLT, vValueGT);
						}
					...
					function resetFiltersHandler() {
						...
						oDialog.getModel().setProperty("/Unidades/vValueLT", "");
						oDialog.getModel().setProperty("/Unidades/vValueGT", "");
					...
					function updateDialogData(filters) {
						var mParams = {
							...
							success: function (oData) {
								...
								oJsonModelDialogData["Unidades"] = {
									vValueLT: (oDialogModel && oDialogModel.getProperty("/Unidades")) ? oDialogModel.getProperty("/Unidades/vValueLT") : "",
									vValueGT: (oDialogModel && oDialogModel.getProperty("/Unidades")) ? oDialogModel.getProperty("/Unidades/vValueGT") : ""
								};
					...
					var designTimeFilters = this.mBindingOptions && this.mBindingOptions[aCollections[0].id] && this.mBindingOptions[aCollections[0]
						.id].filters && this.mBindingOptions[aCollections[0].id].filters[undefined];
		...
		_onButtonPress2: function (oEvent) {
			...
			return new Promise(function (fnResolve) {
				...
				.then(function (result) {
					...
					if (!oDialog) {
						oDialog = sap.ui.xmlfragment({
							fragmentName: "com.sap.build.standard.prototipo1.view.ViewSettingsDialog2"
						}, this);
						oDialog.attachEvent("confirm", confirmHandler);
						oDialog.attachEvent("resetFilters", resetFiltersHandler);
		...
		_onButtonPress3: function (oEvent) {
			...
			return new Promise(function (fnResolve) {
				...
				.then(function (result) {
					...
					if (!oDialog) {
						oDialog = sap.ui.xmlfragment({
							fragmentName: "com.sap.build.standard.prototipo1.view.ViewSettingsDialog3"
						}, this);
						oDialog.attachEvent("confirm", confirmHandler);
						oDialog.attachEvent("resetFilters", resetFiltersHandler);
		...
		onInit: function () {
			...
			this.oRouter.getTarget("EstadoOrdenesDeCompra").attachDisplay(jQuery.proxy(this.handleRouteMatched, this));
		...
		onExit: function () {
			...
			var aControls = [{
				"controlId": "sap_m_Page_0-content-sap_m_ObjectList-1588887640354-nouaf9rif5yv9lqnor526f2b7_S7-uly6wf0jplbqultccjbee1b78_S8-d4bek9cybf5m7nd7i0r8ob0011_S11",
				"groups": ["items"]
			}];
